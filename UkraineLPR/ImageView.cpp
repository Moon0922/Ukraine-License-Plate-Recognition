...
void CImageView::SetImage(HBITMAP hBitmap, CARPLATE_DATA carData)
{
	...
	m_hBitmap = hBitmap;
	GetObject(hBitmap, sizeof(BITMAP), &m_bInfo);
	memset(&m_LPRData, 0, sizeof(CARPLATE_DATA));
	memcpy(&m_LPRData, &carData, sizeof(CARPLATE_DATA));
	UpdateView();
}
...
{
	if (m_LPRData.nPlate <= 0)
		return;
	CPen penRegion(PS_SOLID, 2, RGB(0, 0, 255));
	CPen Selpen(PS_SOLID, 3, RGB(255, 0, 0));
	pDC->SelectStockObject(NULL_BRUSH);
	pDC->SelectObject(&Selpen);
	for (int i = 0; i < m_LPRData.nPlate; i++) {
		int left = m_nLeft + (m_LPRData.pPlate[i].rtPlate.left) * m_fScale;
		int right = m_nLeft + (m_LPRData.pPlate[i].rtPlate.right) * m_fScale;
		int top = m_nTop + (m_LPRData.pPlate[i].rtPlate.top) * m_fScale;
		int bottom = m_nTop + (m_LPRData.pPlate[i].rtPlate.bottom) * m_fScale;
		...
		CString strObjTemp = CString(m_LPRData.pPlate[i].szLicense, strlen(m_LPRData.pPlate[i].szLicense));
		CString temp;
		temp.Format(_T("-[conf: %.2f"), m_LPRData.pPlate[i].nTrust);
		strObjTemp += temp + _T("%]");
